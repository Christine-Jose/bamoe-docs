...
Storage:: This is another deployment. In our case would be PostgreSQL database where you store the process state, jobs, data index, data audit and human task data. There can only be one storage that is shared among components and cannot be divided.
