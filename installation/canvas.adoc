...
NOTE: All services will be available via `https`, which means that if you're deploying them to a local OpenShift instance (that is, via CRC), the {CANVAS} web app, the Extended Services and CORS Proxy backends will all be using self-signed certificates, causing warnings in your browser. To fix this, access each URL individually and allow them in your browser.
...
`KIE_SANDBOX_EXTENDED_SERVICES_URL`:: For example, https://extended-services.bamoe.company.com[https://extended-services.bamoe.my-company.com]

`KIE_SANDBOX_CORS_PROXY_URL`:: For example, https://cors-proxy.bamoe.company.com[https://cors-proxy.bamoe.my-company.com]
...
[NOTE]
====
Those addresses need to be accessible from the your machines, as {CANVAS} will communicate with them directly from the your browser.
Using internal IP/DNS services will not work.
Requests are made directly from the your browser to both URLs configured above.
====
...
For a reference on how to build your own image that is compatible with {CANVAS} development deployments, go to the Apache KIE community's source code for `dmn-dev-deployment-base-image` at https://github.com/apache/incubator-kie-tools/tree/{KIE_TOOLS_COMMUNITY_VERSION}/packages/dmn-dev-deployment-base-image.
====
...
For example, imagine you are creating development deployments for Decisions on {CANVAS} to a local Kubernetes they have installed on their machine, for example Kind or Minikube.
...
For other customization options of {CANVAS}, refer to the https://quay.io/repository/bamoe/canvas?tab=info[description of the image].
