...
You can call this capability of running Decisions while developing them "DMN Runner".
...
When you click the “Run” button, you can see the screen below.
...
If you're familiar with the DMN Boxed Expression Editor, you will be very comfortable navigating through the DMN Runner Table, as they share the same Table component.

DMN Runner will infer the type of your inputs from their Data Types.
To keep your Data Types organized for better maintainability of your Decision, and to extract the most of what DMN Runner has to offer.
...
Therefore, you must build on this example by adding more nodes.

As a first step, move `compute Payroll` node out of the `rate` node and remove its connection with the `taxRate` node.
You can use this node later to invoke the logic wrapped in the Decision Service node.
You can also change the model name from `taxes` to `payroll`.
...
For the Invocation, you need to specify the function name as `payroll` and the single parameter as `employee`, with a passed value of `employee`.
...
There is source code available for the https://raw.githubusercontent.com/bamoe/bamoe-docs-source-code-examples/9.1.x/dmn/payroll.dmn[payroll.dmn] example model.
