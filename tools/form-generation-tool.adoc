...
Download *{FORM_GENERATION_TOOL}* zip file available at https://www.ibm.com/support/pages/node/7151078[BAMOE 9.1.0 Download Document].
The zip file contains three binary files of the same tool for multiple operating systems.
Choose the binary file according your operating system and extract it into desired destination.
Then make sure the binary file is executable.
...
However, for this documentation, you can use existing https://incubator.apache.org/projects/kie.html[Apache Kie,window=_blank] *jbpm-compact-architecture-example* project available at https://www.ibm.com/support/pages/node/7151078[BAMOE 9.1.0 Download Document].
...
Remove `org.jbpm:jbpm-quarkus-devui-bom` from the project `pom.xml`.
...
Now you should see a list of all available forms that listed via the command line earlier in this documentation.
...
----
Click the *Execute form* button to see the result.
