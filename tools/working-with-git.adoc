...
{CANVAS} streamlines the usage of Git to simplify workflows and guide you through a contribution model that minimizes conflicts and mitigates the complexity of managing a Git repository.
...
Although large repositories are supported, you are discouraged from creating them, as {CANVAS} is optimized for working with microservices deployed to a Cloud environment.

== Branches

You can specify the branch to work with when importing a remote Git repository.
...
Instead, it allows you to switch to a new branch temporarily, ignore the conflict, and continue working.

To resolve conflicts externally, you can employ their preferred method and tool, Push the changes, and then return to {CANVAS} and Pull.
